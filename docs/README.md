...
Componente PDFReader, responsable de mostrar un archivo pdf en la store.  
...
```
######  2 - Agregue el pdf-reader component a la página que deseé que renderice este componente.
 ```json
   "pdf-reader": {
    "props": {
      "pdfUrl": "",
...
| -------------- | -------- | ---------------------------------------------------- | ------------- |
| `pdfUrl`     | `String` | URL del archivo PDF    |  N/A |
| `width`     | `number` | Width del lector de PDF    |  N/A |
| `height`     | `number` | Height del lector de PDF    |  N/A |
